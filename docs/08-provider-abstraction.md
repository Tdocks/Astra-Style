...
**A budget is not a deadline, and confusing them cost half the scans.** `closet/handler.ts` set `PROVIDER_TIMEOUT_MS = 5_500` — this section's own "≤ 5.5s p50" — as the hard abort. A p50 is the median, so an abort at the p50 kills about half of all calls by construction, and the measurements above said so before it ever ran against a real garment: p95 4454–5796ms, maximum 5796ms, already past the deadline on an easy photograph. It failed exactly the way that arithmetic predicts — intermittently, on nothing the user did, and worst on the garments that take the model longest to read. Aborting early does not even save money: the provider has begun generating and the tokens are billed whether or not the answer is waited for, so a short deadline pays full price for a result it discards and then pays again when the user rescans. The deadline is now 20s. §2.4's budget is unchanged and remains the thing to measure; a timeout is the point past which something is *wrong* rather than slow, and it can only honestly be set from the tail.
