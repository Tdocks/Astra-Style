...
struct TabNavigationStateTests {
    ...
    func dogfoodChromeHidesUnfinishedTabs() {
        ...
        #expect(router.selectedTab == .studio)
    }

    @Test("startScan opens the scanner, carrying the requested mode")
    func startScanPresentsScanner() {
        let router = AppRouter()

        router.startScan(mode: .batchCloset)

        guard case .scanner(let mode) = router.presentedModal else {
            Issue.record("Expected scanner modal from startScan")
            return
        }
