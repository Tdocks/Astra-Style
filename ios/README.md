...
Some tests in `Tests/UnitTests/PendingIntegrationRequirementsTests.swift` and every test in `Tests/UITests/AstraStyleUITests.swift` are **intentionally failing placeholders** for spec §22 requirements that need real backend/StoreKit/UI infrastructure this scaffold doesn't have yet (a live Supabase project, deployed Edge Functions, a StoreKit sandbox tester, and the still-scaffolded feature screens themselves). Each one names the spec requirement and the ticket prefix expected to close it — do not delete or disable them to make CI green; replace them with real tests as their dependencies land.
